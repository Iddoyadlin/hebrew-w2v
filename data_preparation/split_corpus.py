...
def split(corpus_path: Path, split_corpus_output_folder: Path, split_every: int):
    if not split_corpus_output_folder.exists():
        split_corpus_output_folder.mkdir(parents=True)
    elif list(split_corpus_output_folder.glob('**/*')):
        print(f'there are already files in {split_corpus_output_folder}, skipping splitting corpus')
        return
