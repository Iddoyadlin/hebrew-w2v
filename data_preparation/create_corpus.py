...
def corpus_to_text(corpus_output: Path, dump_path: Path):
    if corpus_output.is_file():
        print(f'skipping creating corpus, file {corpus_output} already exists')
        return
    wiki = WikiCorpus(datapath(str(dump_path)), dictionary={})
    print("Starting to create raw wiki corpus")
    ...
    print(f"created raw wiki corpus at {corpus_output}")
